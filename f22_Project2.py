...
# Name: Mia Friedman
# UM ID: 66734181
# People I worked with: Abhinav Tadikonda
...
.
    ...
    with open(html_file, "r") as inFile:
        ...
        costs = soup.find_all(class_ = '_tyxjp1')
        count = 0
        for y in costs:
            cost_text = y.text
            costs[count] = int(cost_text[1:])
            count += 1

        listing_ids = soup.find_all(class_ = 'c14whb16 dir dir-ltr')
        listing_id_list = []
        pattern = r'(\d+)\?'
        for z in listing_ids:
            item = z.find(class_ = 'rfexzly dir dir-ltr')
            new_item = item.get('href')
            listing_id_list.append(re.findall(pattern, new_item)[0])

        for m in range(0, len(titles)):
            tup_list.append((titles[m], costs[m], listing_id_list[m]))
...
def get_detailed_listing_database(html_file):
    ...
    """
    full_tup_list = []
    title_cost_id = get_listings_from_search_results(html_file)
    for x in title_cost_id:
...
def write_csv(data, filename):
    ...
    """
    info = sorted(data, key = lambda x:x[1])
    f = open(filename, 'w')
    info_csv = csv.writer(f)
    info_csv.writerow(['Listing Title', 'Cost', 'Listing ID', 'Policy Number', 'Place Type', 'Number of Bedrooms'])
    for x in info:
        info_csv.writerow(x)
    f.close()
